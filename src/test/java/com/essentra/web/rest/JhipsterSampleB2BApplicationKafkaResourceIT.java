...
class JhipsterSampleB2BApplicationKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/jhipster-sample-b-2-b-application-kafka/publish?message=value-produce").with(csrf()))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/jhipster-sample-b-2-b-application-kafka/register"))
            .andExpect(status().isOk())
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/jhipster-sample-b-2-b-application-kafka/unregister"));
                return;
            }
        }
